...
public class SecurityConfig {
	...
	*/
	...
	// 1. ADMIN 역할을 가진 사용자만 /books/add로 접근할 수 있도록 등록
	// 2. 로그인 기본 경로 설정, 성공 했을 때의 경로, 실패했을 때의 경로, 로그인 처리에 사용되는 파라미터명(name으로 전달되는 이름) 설정
	...
	protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		// - ADMIN 권한 설정
			.authorizeHttpRequests(authorizeRequests -> authorizeRequests
				.requestMatchers("/books/admin/add")
				.hasRole("ADMIN")
	...
	// 스프링 시큐리티가 회원 인증시에 인증과 권한을 부여하는 프로세스를 처리하는 프로세스를 처리하는 메서드
	@Bean
